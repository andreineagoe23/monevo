...
from gamification.models import (
    Badge, UserBadge, Mission, MissionCompletion, StreakItem, MissionPerformance
)


class MissionAdmin(admin.ModelAdmin):
    """Admin configuration for managing missions with template support and preview."""
    list_display = ('name', 'mission_type', 'goal_type', 'points_reward', 'is_template', 'preview_link')
    fields = (
        'name', 'description', 'points_reward', 'mission_type', 'goal_type', 'goal_reference',
        'is_template', 'purpose_statement', 'target_weakest_skills',
        'min_difficulty', 'max_difficulty', 'fact'
    )
    ...
    def preview_mission(self, request, mission_id):
        ...
        context = {
            ...
            'purpose_statement': mission.purpose_statement or "Completing this mission keeps your learning loop tight.",
        }
        
        return TemplateResponse(
            request,
            'admin/gamification/mission_preview.html',
            context
        )
...
class MissionCompletionAdmin(admin.ModelAdmin):
    ...
    list_display = ('user', 'mission', 'progress', 'status', 'xp_awarded', 'completed_at', 'first_try_bonus', 'mastery_bonus')
    fields = (
        'user', 'mission', 'progress', 'status', 'completed_at',
        'swapped_at', 'swapped_from_mission',
        'first_try_bonus', 'mastery_bonus', 'xp_awarded',
        'completion_time_seconds', 'completion_idempotency_key'
    )
