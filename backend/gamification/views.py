    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                mission_completion = MissionCompletion.objects.get(
                    user=user,
                ...
                if request.data.get("completion_time_seconds"):
                    mission_completion.completion_time_seconds = request.data.get("completion_time_seconds")
...
class MissionSwapView(APIView):
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                if swapped_today:
                    return Response({
                        "error": "You can only swap one mission per day."
                    }, status=400)
                
                if mission_completion.status == 'completed':
                    return Response({
                        "error": "Cannot swap a completed mission."
                    }, status=400)
                
                # Generate a new mastery-aware mission
                new_mission = self._generate_mastery_aware_mission(
                    user,
                    mission_completion.mission.mission_type
                )
    ...
    def _generate_mastery_aware_mission(self, user, mission_type):
        ...
        weakest_skills = Mastery.objects.filter(
            user=user
        ).order_by('proficiency', 'due_at')[:3]
    ...
    def post(self, request):
        ...
        try:
            with transaction.atomic():
                ...
                if not item:
                    return Response({
                        "error": f"No {item_type} items available."
                    }, status=400)
                ...
                if item_type == 'streak_freeze':
                    ...
                    user_profile = user.profile
                    today = timezone.now().date()
                    if user_profile.last_completed_date and user_profile.last_completed_date < today:
...
class MissionAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        # Calculate metrics
        avg_completion_time = performances.aggregate(
            avg_time=Avg('time_to_completion_seconds')
        )['avg_time'] or 0
